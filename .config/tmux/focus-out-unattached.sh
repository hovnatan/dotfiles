...
# client-session-changed hooks with #{socket_path}; writes ESC [ O (the
# focus-out sequence, hex 1b 5b 4f) into every Claude Code pane of every
# session no client is attached to. Repeats are harmless, and attaching
# again delivers a real focus-in.
set -u
