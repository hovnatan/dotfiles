...
local function profileName(profileDir)
  local state = hs.json.read(os.getenv("HOME") .. "/Library/Application Support/Google/Chrome/Local State")
...
local function selectProfile(app, profileDir)
  if menuTitles[profileDir] and app:selectMenuItem({ "Profiles", menuTitles[profileDir] }) then
    return
  end
...
local function focusChromeProfile(profileDir)
  ...
  app:activate()
  if profileDir then
    selectProfile(app, profileDir)
  end
end

local MOD = { "alt" }
hs.hotkey.bind(MOD, "1", function()
  focusChromeProfile(cfg.default_chrome_profile)
end)
hs.hotkey.bind(MOD, "2", function()
  focusChromeProfile(cfg.work_chrome_profile)
end)
