...
sudo pacman -Sy python-pip python-sh python-language-server autopep8 python-pyperclip ipython python-pydbus python-systemd python-unidecode
...
yay -S --needed skype wd719x-firmware aic94xx-firmware zoom i3ipc-python-git libinput-gestures clipster xcwd-git foxitreader i3lock-color gruvbox-icon-theme bear-git nerd-fonts-dejavu-complete ripgrep-all python-snakeviz subliminal vte3-ng dragon-drag-and-drop-git google-chrome ptags xkb-switch-git flacon freeoffice mpv-mpris-git playerctl-git ranger-git autojump python-pynput
sudo grub-mkconfig -o /boot/grub/grub.cfg
