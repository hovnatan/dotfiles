...
if __name__ == "__main__":
    ...
    with progress.Progress(
        ...
        progress.TextColumn("aa {task.fields[part_id]}"),
        ...
        with multiprocessing.Manager() as manager:
            ...
            overall_progress_task = progress.add_task(
                "[green]All jobs progress:", part_id="rr2"
            )
            ...
            with ProcessPoolExecutor(max_workers=n_workers) as executor:
                ...
                for n in range(0, 20):  # iterate over the jobs we need to run
                    ...
                    task_id = progress.add_task(
                        f"task {n}", visible=False, part_id="rr"
                    )
                ...
                while (n_finished := sum([future.done() for future in futures])) < len(
                    ...
                    for task_id, update_data in _progress.items():
                        ...
                        # progress.print("hhh", task_id, latest)
                        progress.update(
                            task_id,
                            completed=latest,
                            total=total,
                            visible=latest < total,
                            part_id=f"rrrr {latest}",
                        )
